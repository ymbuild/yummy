...
| `ym env`（无项目） | < 10ms | — |
| 加载 `package.json` | < 1ms | Gradle 配置阶段 20s+ |

### 依赖解析

| 场景 | 目标 | 条件 |
|------|------|------|
| 锁文件命中 + 缓存命中 | < 500ms（1000 模块） | 所有 JAR 在本地 |
| 锁文件命中 + 缓存未命中 | < 30s | 需要下载 JAR |
| 全新解析（无锁文件） | < 60s | 需要下载 POM + JAR |
| 单模块增量（1 个依赖变更） | < 3s | — |
...
| 编译 → 热重载生效 | < 200ms | HotSwap (L1) |
| 编译 → ClassLoader 重载 | < 500ms | L2 |
| 编译 → 进程重启 | < 3s | L3 |
...
```bash
# 冷启动全量编译
hyperfine --warmup 1 --runs 5 'ymc rebuild'
